...
def mark_emails_as_read(session: AuthorizedSession, response_message_ids: list[str]) -> None:
    """Mark all specified emails as read if any."""
...
def parse_message_info(full_message: dict[str, Any]) -> dict[str, str]:
    '''Extracts and returns {"email": email, "body": body, "date": full_date } from transaction'''
...
def get_message_info(session: AuthorizedSession, message_id: str) -> dict[str, str]:
    ...
    return parse_message_info(full_message)


def get_all_message_info() -> list[dict[str, str]]:
    """Get a list of user email, body, and date of all unread emails."""
    ...
    response_messages.reverse()  # Reverses so that they are in chronological order
    ...
    for message in response_messages:
        ...
        message_info = get_message_info(session, message_id)

        all_message_info.append(message_info)
        response_message_ids.append(message_id)

    if ENV == "prod":
        mark_emails_as_read(session, response_message_ids)
        # TODO: Add better error handling (Mark as read after categorisation is done)
